    ...
    def run(self):
        ...
        while True:
            for message in self.slack_client.rtm_read():
                if message.get('type', False) == 'message' and message.get('channel', False) == self.channel:
    ...
    def confirm(self, message):
        ...
        try:
            ...
            with db.transaction():
                ...
                winner_transformed_rating = 10**(match.winner.rating/400)
                loser_transformed_rating  = 10**(match.loser.rating/400)
                ...
                match.winner.rating = match.winner.rating + match.winner.k_factor() * (1 - winner_expected_score)
                match.loser.rating = match.loser.rating + match.loser.k_factor() * (0 - loser_expected_score)
                ...
                self.talk('<@' + match.winner.slack_id + '> your new ELO is: ' + str(winner_old_elo) + ' (+' + str(match.winner.rating - winner_old_elo) + ')')
                self.talk('<@' + match.loser.slack_id + '> your new ELO is: ' + str(loser_old_elo) + ' (' + str(match.loser.rating - loser_old_elo) + ')')
        except Exception as e:
            print(e)
            self.talk('Unable to confirm ' + values[1])
